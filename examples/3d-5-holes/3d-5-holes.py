...
class BorrvallProblem(PrimalInteriorPoint):
    ...
    def initial_guesses(self, Z, params):
        ...
        solve(F == 0, g, bcs = self.Gbcs, solver_parameters = solver_params, nullspace=nsp)
        ...
        info_green("Initial guess projected, saving ...")
        info_green("Initial guess computed.")
        return [z]
    ...
    def number_solutions(self, mu, params):
        # Prescribe where to find the solutions
        if float(mu) > 55:
            return 1
        elif 11.4 < float(mu) < 55:
            return 3
        elif 10.5 < float(mu) <=11.4:
            return 7
        elif 10 < float(mu) <=10.5:
            return 8
        elif 6.8 < float(mu) <=10:
            return 9
        elif 6.2 < float(mu) <=6.8:
            return 11
        elif 4.5 < float(mu) <= 6.2:
            return 13
        else: return 14
    ...
    def solver_parameters(self, mu, branch, task, params):
        ...
        args = {
                ...
                "snes_divergence_tolerance": 1e1000,
                "snes_monitor": None,
...
if __name__ == "__main__":
    ...
    params = [0.2, 2.5e4, 0.1] #(gamma, alphabar, q) # parameters for the BorrvallProblem

    # Run the deflated barrier method algorithm
    solutions = deflatedbarrier(problem, params, mu_start=200, mu_end = 1e-10, max_halfstep = 0)
