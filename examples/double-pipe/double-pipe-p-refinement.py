...
"""
This script implements a p-refinement for the double-pipe problem with
a Brezzi-Douglas-Marini discretization for the velocity-pressure pair.
...
class BorrvallProblem(PrimalInteriorPoint):
    ...
    def function_space(self, mesh):
        # (Integral-evaluated) p-order BDM discretization for the velocity
    ...
    def initial_guesses(self, Z, params):
        """
        Use first-order BDM solutions as initial guesses.
        """
        comm = Z.comm
        z = []
        info_blue("Loading initial guesses.")
        folder = "initial-guess-%sgrid-p-%s/"%(nref+1, porder)
        for branches in range(2):
            z.append(Function(Z))
            h5 = HDF5File(folder + "%s.xml.gz"%branches, "r", comm=comm)
            h5.read(z[branches], "/guess")
            del h5


        info_blue("Successfully loaded")
        return z
...
if __name__ == "__main__":
    ...
    # 2-grid cycle for augmented momentum block.
    N = 50 # mesh resolution
    nref = 1 # mesh refinement level (1 for 2-grid MG)
    for porder in range(2,5):
       saving_folder = "p-refinement-%s-aL2-N-%s-nref-%s-"%(porder, N, nref)
       solutions = deflatedbarrier(problem, params, mu_start=0., mu_end = 1e-5, max_halfstep = 0, saving_folder=saving_folder)

    # Uncomment (and comment above) for 3-grid cycle for augmented momentum block
    # N = 25 # mesh resolution
    # nref = 2 # mesh refinement level (2 for 3-grid MG)
    # for porder in range(2,5):
    #     saving_folder = "p-refinement-%s-aL2-N-%s-nref-%s-"%(porder, N, nref)
    #     solutions = deflatedbarrier(problem, params, mu_start=0., mu_end = 1e-5, max_halfstep = 0, saving_folder=saving_folder)
